...
define("LIVE_IO", [], function(require, exports, module) {
    (function(global, undefined) {
        ...
        (function(util) {
                ...
                function matchVersions() {
                    ...
                    if (rl > 0) for (var i = 0; i < rl; i++) {
                        ...
                        if (ua.pv[0] > 0) {
                            ...
                            if (obj) if (hasPlayerVersion(regObjArr[i].swfVersion) && !(ua.wk && ua.wk < 312)) setVisibility(id, !0), cb && (cbObj.success = !0, cbObj.ref = getObjectById(id), cb(cbObj));
            ...
            function jsonp(url, data, callback) {
                ...
                if (data) for (var k in data) param += "&" + k + "=" + encodeURIComponent(data[k]);
        ...
        }(liveIO, liveIO.util), function(liveIO, util) {
    ...
    var $ = require("JQ"),
        ...
        LINK_GIFT = X.XM.extend({
            _constructor: function(option) {
                return this._container = option.container, this._giftView = null, this._Event_Name = "gift_link_", this._sendGiftCallback = option.sendGiftCallback ||
                function() {}, this
            },
            init: function() {
    ...
    var TaskView = X.XM.extend({
        ...
        RoomTask_view = SimpleTaskView.extend({
            ...
            _createHTML: function() {
                var html = ['<li class="item task-item task-item-dsc">', '<p class="title-line line">', '<span class="status">[加载中]</span>', '<span class="name" title=""></span>', '<span class="count"></span>', "</p>", '<p class="line dsc"><span class="text"></span></p>', '<p class="line info">倒计时<span class="time">00:00:00</span>，当前进度[<span class="s-count">0/0</span>]</p>', function(mode) {
                    return mode != "anchor" ? '<span class="rec-btn btn-red"><span class="text"></span></span>' : ""
                }(this.mode), '<p class="line status-text" style="color:#FFF;">未开启</p>', '<p class="line empty" style="color:#FFF;">今日已达开启上限</p>', "</li>"].join("");
                return html
            },
        ...
        OptionTaskAddBtn = X.XM.extend({
            ...
            _openSelect: function() {
                ...
                if (data.length == 0) D.alert("暂时没有可添加的任务");
                    ...
                    function getContent() {
                        var html = ['<ul class="MR-community-task-select">', function(d) {
                            var html = "";
            ...
            openOption: function(pid) {
                ...
                function getContent(d) {
                    var html = ['<div class="MR-community-task-select-form">', '<h5 class="title">' + d.name + "</h5>", '<p class="desc">' + d.desc + "</p>", function(d) {
                        var html = "";
                        return d.ty == 5 && (html += '<h6 class="gift-title">选择礼物</h6>', html += '<div class="gift-container"></div>'), html
                    }(d), '<p class="input">', '<input class="price" placeholder="填写完成任务额外奖励，如唱歌、跳舞、演奏、红包、实物奖励等(选填、限10个汉字)"/>', "</p>", '<div class="btn-area clearfix">', '<span class="btn btn-add">添加</span>', '<span class="btn btn-cancel">取消</span>', "</div>", "</div>"].join("");
                    return html
                }
                var select = D.dialog({
                    title: "",
    ...
    var Coffer = X.XM.extend({
        ...
        update: function(startTime, sysTime, effectiveTime) {
            ...
            if (this.view.btn && this.view.btn.length > 0) if (startTime == 0) this.setBtn(!0);
            else {
                this.setBtn(!1);
    ...
    var SenderView = X.XM.extend({
        ...
        Sender = X.XM.extend({
            ...
            update: function(d) { !! d.a && !! d.l && (d.o == 1 && this._count > d.t || d.o != 1) && (this._count = d.t), d.pc > -1 && (this._payCount = d.pc), d.p > 0 && (this._price = d.p), this.view.setCount(this._count), this.view.setBuyCount(this._payCount)
            },
            show: function() {
                this.view.show()
            },
    ...
    var X = require("XM"),
        ...
        VIDEO = liveIO.util.Listener.extend({
            ...
            init: function() {
                ...
                return _TYPE_REG.lastIndex = 0, this._clientType = typeArr[1], this._uiType = typeArr[2], o.isAnchor ? this._videoContainer = this._container.find("#live-player-window") : this._isTv ? this._videoContainer = this._container : this._isV2 ? (this._videoContainer = this._container, this._isChromeFlash && (this._videoContainer = $('<div class="channel-player-v2"></div>'), this._container.append(this._videoContainer))) : (this._videoContainer = $('<div class="MR-video"></div>'), this._container.append(this._videoContainer)), this._videoContainer.append('<div class="con" id="' + this._id + '"></div>'), this._isIku && (o.src = o.src + "?_=" + (new Date).getTime()), this.player = player = this._initPlayer(this._clientType, o), this.setChatRate("chatRate", window.DDS.baseInfo.chatRate), this._clientType == "html5" && (this.panel = this._videoContainer), player && (this._clientType == "html5" && this._uiType && (player._controlBar = !1), player.on("launchSuccess", function() {
                    _this._uiType && _this._initUI(_this._uiType), window.DDS.baseInfo.isShowing && o.initAlias && o.initToken ? _this.startLiveByAlias(o.initAlias, o.initToken) : (player._customUI && (player._customUI.controlBar.hide(), player._customUI.bigBtn.hide()), _this._isTv && o.initStopType != undefined && _this.stopLive(o.initStopType)), _this._isChromeFlash && (_this._videoContainer.attr("chromeQ", "1"), _this._videoContainer.css({
                        top: 0,
                        left: 0,
                        width: _this._container.width(),
                        height: _this._container.height()
                    })), _this.emit("launchSuccess")
    ...
    msg.prototype = {
        ...
        getTopIcon: function(id, args) {
            if (!this.isAnchor(id) && args.md && args.md.gws && args.md.gws.length > 0) {
                var html = "",
                    data = args.md.gws;
                for (var i = 0; i < data.length; i++) if (this.data.gift[data[i]]) {
    ...
    talk.prototype.init = function() {
        ...
        this.chatCache = arr;
        for (var i = 0, len = this.chatCache.length; i < len; i++) if (this.chatCache[i].value == value) return !0;
    ...
    SiteHornMsg.prototype = {
        ...
        getNextArr: function(arr) {
            for (var i = 0; i < this.msgArr.length; i++) if (arr.index == this.msgArr[i].index) return this.msgArr[i + 1] ? this.msgArr[i + 1] : null
        },
        fx: function(args) {
            var _this = this;
            this.TOGGLE = !1;
    ...
    BonusAccept.prototype = {
        ...
        cssSupports: function(prop) {
            ...
            while (len--) if (vendors[len] + prop in div.style) return !0;
            return !1
        },
    ...
    Online.prototype = {
        ...
        createOnLine: function() {
                ...
                m_name = "社团管理员",
                u_name = "在线用户";
                this.isException && window.DDS.config.roomType != "tv" && (str = "<br/>"),
                window.DDS.config.roomType == "tv" && (m_name = "管理员", u_name = "用户"),
                this.onlineList = $('<div class="dialog"></div>'),
                this.panel.append(this.onlineList),
                this.online_tab = $('<ul class="nav-tab"></ul>'),
                this.online_con = $('<div class="nav-con"></div>'),
                this.onlineList.append(this.online_tab).append(this.online_con),
                this.admin_li = $("<li>" + m_name + str + "(</li>"),
                this.admin_count = $("<cite></cite>"),
                this.user_li = $('<li class="end">' + u_name + str + "(</li>"),
                this.user_count = $("<cite></cite>"),
                this.admin_con = $('<div class="admin-con"></div>'),
                this.user_con = $('<div class="user-con"></div>'),
                this.more_toggle = $('<div class="more-toggle"></div>'),
                this.admin_con.css("display", "none"),
                this.user_li.addClass("on"),
                this.admin_li.append(this.admin_count),
                this.admin_li.append(")"),
                this.user_li.append(this.user_count),
                this.user_li.append(")"),
                this.isException && this.createExc(),
                this.online_tab.append(this.user_li).append(this.admin_li),
                this.online_con.append(this.user_con).append(this.admin_con),
                this.onlineList.on("click", function(e) {
                    e.stopPropagation()
                });
            var _this = this;
            this.admin_scroll = Widgets.ScrollBoard({
                ...
                skin: "dds-scroll-nav"
            }), this.user_scroll.init(),
                this.user_con.append(this.more_toggle),
                this.admin_li.bind("click", function() {
                    var ele = $(this);
                    if (ele.hasClass("on")) return !1;
    ...
    Square.prototype = {
        ...
        getSquareList: function() {
            ...
            con.empty(), Util.get(this.URL_Square, {}, function(data) {
                ...
                else for (var i = 0; i < D.length; i++) con.append(_this.getOneUserList(D[i]));
    ...
    var X = require("XM"),
        ...
        View = X.XM.extend({
            ...
            _createHTML: function(isAnchor) {
                var html = ['<div class="MR-guard-container">', '<div class="top-container">', '<h6 class="title">Ta的守护</h6>', '<span class="count">0/32</span>', "</div>", '<div class="list-container"><div class="list-inner"></div></div><div class="scroller"></div>', function(flag) {
                    var res = "";
                    return flag || (res = '<div class="bottom-container"><span class="buy white-btn">购买守护</span></div>'), res
                }(isAnchor), '<div class="notice"></div>', "</div>"].join("");
                return html
            },
            _createItemHTML: function(d) {
                var html = ['<li class="item', function(self) {
                    var res = "";
                    return self && (res = " item-self"), res
                }(d.self), '">', '<span class="pic"><img src="' + d.f + '" alt="' + d.n + '"/></span>', '<span class="name" title="' + d.n + '" data-cardid="' + d.u + '">' + U.cutZhFont(d.n, 6) + "</span>", function(online) {
                    var res = "";
                    return online || (res = '<span class="off-line"></span>'), res
                }(d.io), '<span class="limit">还剩<br/><i style="color:#F00;">', function(hour) {
                    var res = " - ";
                    return hour != undefined && (hour == 0 ? res = "<1小时" : hour < 24 && hour > 0 ? res = hour + "小时" : res = Math.floor(hour / 24) + "天"), res
                }(d.lh), "</i></span>", function(year) {
                    var res = "";
            ...
            showNotice: function(param) {
                var notice = this.notice,
                    html = ['<p class="pic"><img src="' + param.f + '" /></p>', '<p class="name"><span class="icon ICON-noble-level ICON-nl-' + param.l + '"></span><span class="text">' + param.n + "</span></p>", '<p class="msg">为<span class="hl">' + param.anchorName + "</span>开通了<br/><b>", function(y) {
                        var res = "";
                        return y && (res = "年"), res
                    }(param.y), "守护</b></p>"].join("");
    ...
    var X = require("XM"),
        ...
        Buy = X.XM.extend({
            _constructor: function(option) {
                return this._anchorId = option.anchorId, this.view = new BuyView(option.container), this.module = new BuyModule, this._initType = option.initType, this._pricePerMonth = option.pricePerMonth, this._monthCount = 0, this._price = 0, this._onSuccess = option.onSuccess ||
                function() {}, this._onComplete = option.onComplete ||
                function() {}, this._onError = option.onError ||
                function() {}, this._btnText = option.btnText || "购买", this._discount = option.discount, this._discountPercent = .85, this._type = this._initType, this
            },
            init: function() {
                var _this = this;
        ...
        BuyDialog = X.XM.extend({
            _constructor: function(option) {
                ...
                function() {}, this._onComplete = option.onComplete ||
                function() {}, this._onError = option.onError ||
                function() {}, this._discount = option.discount, this._btnText = option.btnText, this
            },
    ...
    SingleChat.prototype = {
        ...
        removeUser: function(uid) {
            ...
            var users = this.cache.users;
            for (var i = 0; i < users.length; i++) if (users[i].id == uid) {
                users.splice(i, 1);
                break
            }
        },
    ...
    packages.prototype = {
        ...
        getGiftExpire: function(ele) {
            ...
            for (; i < len; i++) if (packGoods[i].gid == gid) return packGoods[i].product.expire;
            return 0
        },
        getOneGoods: function(ele) {
            var gid = ele.attr("data-gid"),
                ...
                i = 0,
                len = packGoods.length;
            for (; i < len; i++) if (packGoods[i].gid == gid) return packGoods[i].product
        },
    ...
    var $ = require("JQ"),
        ...
        RoomSofa = X.XM.extend({
            _constructor: function(o) {
                return this.view = new RoomSofa_View(o.container), this.module = new RoomSofa_Module, this._sofas = {}, this._isAnchor = !! o.isAnchor, this._roomId = o.roomId, this._selfId = o.selfId, this._hugGift = o.giftData[window.DDS.config.sofaGift[0].data[0].id], this.sendChatMsg = o.sendChatMsg ||
                function() {}, this._emptyMsgTimer = null, this._MY = o.my, this._isOpen = !0, this
            },
            init: function() {
                var _this = this;
    ...
    listener.prototype = {
        ...
        listeners: function() {
            ...
            this.my = this.defineModel("my"), this.video = this.defineModel("video"), this.talk = this.defineModel("talk"), this.msg = this.defineModel("msg"), this.comboMsg = this.defineModel("comboMsg"), this.gift = this.defineModel("gift"), this.about = this.defineModel("about"), this.horn = this.defineModel("horn"), this.bonus = this.defineModel("bonus"), this.count = this.defineModel("count"), this.giftcount = this.defineModel("giftcount"), this.osnotice = this.defineModel("osnotice"), this.roomnotice = this.defineModel("roomnotice"), this.online = this.defineModel("online"), this.biggift = this.defineModel("biggift"), this.countdown = this.defineModel("countdown"), this.appManager = this.defineModel("appManager"), this.appSong = this.defineModel("appSong"), this.appMulti = this.defineModel("appMulti"), this.appVote = this.defineModel("appVote"), this.appSetting = this.defineModel("appSetting"), this.guard = this.defineModel("guard"), this.ticket = this.defineModel("ticket"), this.singleChat = this.defineModel("singleChat"), this.packages = this.defineModel("packages"), this.berth = this.defineModel("berth"), this.sofa = this.defineModel("sofa"), this.giftLink = this.defineModel("giftLink"), this.communityPanel = this.defineModel("communityPanel"), this.tvInfo = this.defineModel("tvInfo"), this.micro = this.defineModel("micro"), this.plugin = this.defineModel("plugin"), this.anchorMicroNotice = this.defineModel("anchorMicroNotice"), this.valentine = this.defineModel("valentine"), this.countDownTimer = null, this.socket.on("flashinfo", function(args) {
                ...
                if (_this.horn) if (_this.horn.config && _this.horn.config.isFlash) {
                    var _flash = $("#ddshowPlayer")[0];
                    if (_flash) try {
                        _flash.setChatInfo({
                            type: 2,
        ...
        openLevelUpDialog: function(data) {
            ...
            html += '<div class="M-level-up-dialog">', html += '<p class="level-name"><span class="level-icon level-icon-' + level + '"></span>' + levelName + "</p>", html += '<p class="level-msg">哇！恭喜您荣升<span class="h">' + levelName + "</span></p>", html +=
                function(rightData) {
        ...
        _redPacketDialog: function(body) {
            var coin = body.coin,
                ...
                html = "";
            if (coin <= 0) return !1;
            coin >= 3e4 ? tag = "big" : coin >= 3e3 ? tag = "middle" : tag = "small", html = ['<div class="M-spring-dialog-container ' + option[tag].skin + '">', function(tag) {
                var html = "",
                ...
                return html
            }(tag), '<p class="coin-count">' + coin + "<small>星币</small></p>", "</div>"].join(""), Dialog.dialog({
                title: "",
