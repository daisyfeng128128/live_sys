...
<ul id="tags">
<li style="display:none;"><a href="javascript:void(0)" onclick="selectTag('tagContent0',this)" id="tagContent0Btn"><?php echo $page_var['money_name2']?>兑换<?php echo $page_var['money_name']?></a></li>
...
<li class="selectTag"><a href="javascript:void(0)" onclick="selectTag('tagContent2',this)" id="tagContent2Btn"><?php echo $page_var['money_name2']?>提现</a></li>
</ul>
</div>
<div id="tagContent">
<div class="tagContent" id="tagContent0">
<div class="zhyk">您的账户余额:<em class="hfb"><?php echo $page_var['money_name']?><span id="selfbalance"><?php echo (int)$user['balance']?></span></em><em class="jf"><?php echo $page_var['money_name2']?><span id="selfpoint"><?php echo (int)$user['point']?></span></em><em class="yxb">游戏币<span id="selfyxb"><?php echo (int)$user['gamemoney']?></span></em></div>
<div>
...
<div class="zhyk">您的账户余额:<em class="hfb"><?php echo $page_var['money_name']?><?php echo (int)$user['balance']?></em><em class="yxb">游戏币<?php echo (int)$user['gamemoney']?></em></div>
<div>
...
<div class="zhyk">您的账户余额:<em class="hfb"><?php echo $page_var['money_name']?><?php echo (int)$user['balance']?></em></div>
<div>
<h5><span class="dhyxbtitle"><?php echo $page_var['money_name']?>提现</span></h5>
<form>
<ul class="dhjf">
  <li><span class="dhjftxt">需兑换的<?php echo $page_var['money_name']?>数量：</span>
  <span>
  ...
  <li class="attention_1">提示：主播可以将自己获得的<?php echo $page_var['money_name']?>按<?php echo JF_TIXIAN?>:1的比例兑换成人民币 <?php echo JF_TIXIAN?><?php echo $page_var['money_name']?>=1RMB。每次兑换不得低于<?php echo XNB_TI_MIN?><?php echo $page_var['money_name']?>。</li>
</ul>
<br/><br/><br/>
...
$("#exchange3btn").click(function(){
	if(parseInt($("#exchange3value").val())>parseInt($("#selfbalance").html())){
		alert("您没有那么多<?php echo $page_var['money_name']?>");
		return;
	}
	if(!$.isNumeric($("#exchange3value").val())){
		alert("请输入正确的<?php echo $page_var['money_name']?>值");
		return;
	}
	if(parseInt($("#exchange3value").val())<<?php echo XNB_TI_MIN?>){
		alert('<?php echo XNB_TI_MIN?><?php echo $page_var['money_name']?>起兑');
		return;
	}
	if(!confirm("确认进行此操作吗?"))return false;
