<link rel="stylesheet" href="<?php echo $page_var['cdn_domain']?>/css/memberQ2v1.4.css?20150522" type="text/css" media="screen">
<link rel="stylesheet" href="<?php echo $page_var['cdn_domain']?>/css/mallv1.5.css" type="text/css" media="screen">
<script src="<?php echo $page_var['cdn_domain']?>/js/common-nsr0.8.js?20150522"></script>
<script type="text/javascript" src="<?php echo $page_var['cdn_domain']?>/js/mallQ2v1.6.6.js?20150522"></script>
...
<?php endif;?>
				...
				<ul class="recharge_menu" id="recmenu">
					...
					<li class="icon_rec_txt" data-rel="rec_ips" data-rectype="环讯">环讯</li>
				</ul>
			</div>
		</div>
		<!--右边加载区域-->
...
<li class="mrBk"><input type="radio" name="bank" id="qitayinhang" value=""/><a href="javascript:void(0)"><label class="qitayinhang" for="qitayinhangyinhang"></label></a></li>
				...
				<div class="rec_ips rec_info" style="display: block;">
					<!--文字提示-->
					...
					<div class="base_info" style="height:210px; border-bottom:1px dashed #eee;">
						...
						<p>请选择充值金额：
							<input type="radio" name="chargtypeips" value="0" checked="checked" onchange="selectRmbips();">
							<select id="selectRmbips" onchange="selectRmbips();">
								<option value="10">10</option>
								<option value="20">20</option>
								<option value="30">30</option>
								<option value="50" selected="selected">50</option>
							...
							&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" name="chargtypeips" value="1" onchange="$('#selectRmbexips').blur()">&nbsp;其他金额&nbsp;&nbsp;<input type="text" id="selectRmbexips" class="intBd input180 " style="width:90px">元
						</p>
						<p>可得到<?php echo $page_var['money_name']?>：<input type="text" class="colorfo input_special" value="0" id="orderAmountips" disabled="true"></p>
						<div class="color999">兑换比例：人民币和<?php echo $page_var['money_name']?>兑换比例1:<?php echo RMB_XNB?></div>
					</div>
					<!--选择银行-->
					<div class="choose_bank">
						<div class="clear"></div>
						<button class="btn_porange" id="gotoBankips" style="margin:20px 0 0 300px;">充&nbsp;&nbsp;值</button>
					</div>
				</div>
			</div>			
		</div>
...
$(".btn_porange").click(function(){
	...
	else if(pgname=='rec_ips'){//ips
		url="/apis/ips/redirect.php";
		var bankid=$('input[name=chargtypeips]:checked').val();
		if(bankid=="0")//请选择充值金额
			var cash = $("#selectRmbips").val();
		else
			var cash = $("#selectRmbexips").val();
		if(!cash){
			Main.alert('请输入正确的充值金额');
			return;
		}
