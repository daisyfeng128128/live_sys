...
function changeVaildcodeLogin(){
	$(".yzmimg_login").attr('src','/tools/captcha.php?loginCode=login&t='+getTimestamp());
}
function _login(){
...
　　  	window.open('/opensns/qq/index.php','snslogin','top='+(window.screen.height-h)/2+',left='+(window.screen.width-w)/2+',width='+w+',height='+h);
	...
	thirdlogin:function(){
		...
		var w=500;
		var h=400;
　　	window.open('/opensns/third/index.php?action=login','snslogin','top='+(window.screen.height-h)/2+',left='+(window.screen.width-w)/2+',width='+w+',height='+h);
	},
	thirdreg:function(){
		this.close();
		var w=500;
		var h=400;
　　	window.open('/opensns/third/index.php?action=reg','snslogin','top='+(window.screen.height-h)/2+',left='+(window.screen.width-w)/2+',width='+w+',height='+h);
	},
	sinalogin:function(){
		this.close();
		var w=500;
...
$(function(){
	...
	$(".tcloginbtn").click(function(){
		if($("#tcuser").val()==''){
			...
			$.post('/login.php',{username:$("#tcuser").val(),'password':$("#tcpass").val(),'login_vaildcode':$("#login_vaildcode").val(),'remember':$("#tcremember").attr('checked')},function(r){
				if(r.r==100){
	...
	$(".regbtn").click(function(){
		$.post('/register.php',{action:"reg",email:$("#reg_email").val(),password:$("#reg_password").val(),repassword:$("#reg_repassword").val(),nickname:$("#reg_nickname").val(),vaildcode:$("#reg_vaildcode").val()},function(r){
			if(r.r=='yes'){
				...
				// alert(r.r);
				$(".border_center .checkCode").html("您输入的验证码有误").show();
			}
		},'json');
	});
