...
$(function () {
    SwapTab(".anch .tit", "a", ".anch .anglisg", ".anchList", "active");
    SwapTab(".richers .tit", "a", ".richers .anglisg", ".richList", "active");
...
$(function(){
    ...
    <img class="fl" src="{1}" alt="{2}"/>\
    <span class="fl deg diamond fl {3}"></span>\
    <div class="fl entry">\
        <p class="name">{4}</p>\
    </div>\
    </a>\
    ...
    Tools.getJson({
        url: '/files/giftWeels.json',
        data: ""
    }, function (data) {
        data=JSON.parse(data);
        if (data.data != null && data.data.length > 0) {
            $.each(data.data, function (k, v) {
                s_ht += Tools.stringFormat(ht_rank, v.roomNumber,v.avatar,v.truename,v.levelImg,v.truename);
                if (k == 5) {
                    return false;
                }
            })
    ...
    Tools.getJson({
        url: '/files/localTyrantMonth.json',
        data: ""
    }, function (data) {
        s_ht='';
        data=JSON.parse(data);
