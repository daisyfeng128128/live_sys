...
$(document).ready(function(){
    ...
    $(function () {
                ...
                if (data != null) {
                    ...
                    $.each($listObj, function() {
                        ...
                        if(window.navigator.userAgent.indexOf("MSIE")>=1){
                            _self.roomNumber ="/guide.php?roomnumber=" + _self.roomNumber;
                        }
        ...
        Mycare.addEvent=function(){
            ...
            $('.titD').live('click','.glyphicon-menu-down',function(){
                if(canBottomNum>0){
            ...
            $('.titD').live('click','.glyphicon-menu-up',function(){
                $('.follow .caption .boxx').animate({top:scrollerTop + li_len * 5}, 300);
                scrollNum--;
                canBottomNum++;
                setScrollRange(-scrollNum*334);
                ...
                if (data.data != null && data.data.length > 0) {
                    ...
                    $.each(data.data, function(k, v) {
                        if(type == "anchor"){
                            ...
                            if(window.navigator.userAgent.indexOf("MSIE")>=1){
                                roomnumber ="/guide.php?roomnumber=" + v.roomNumber;
                            }
                        }
            ...
            Tools.getJson({
                ...
                if (datas != null && datas.length > 0) {
                    ...
                    $.each(row1, function(k, v) {
                        ...
                        if(window.navigator.userAgent.indexOf("MSIE")>=1){
                            v.roomNumber ="/guide.php?roomnumber=" + v.roomNumber;
                        }
                        if(k==0){
            ...
            Tools.getJson({
                ...
                if (dealData != null && dealData.length > 0) {
                    ...
                    $.each(row1, function(k, v) {
                        ...
                        if(k==0){
                            s_big = Tools.stringFormat(compiliter.bightmlNew,v.roomNumber,v.image,ndecodeURI(v.nickName), ndecodeURI(v.nickName),v.numbers,v.heats);
                            bwrap.append(s_big);
                            bwrap.append(advertisement);
                        }else if(k>0 && k<=6){
                            s_sml = Tools.stringFormat(compiliter.lithtmlNew,"col-lg-2 col-md-3 col-sm-3 col-xs-6",v.roomNumber,v.image, ndecodeURI(v.nickName),ndecodeURI(v.nickName),v.numbers);
                        }else if(k>6 && k <=12){
                            s_sml = Tools.stringFormat(compiliter.lithtmlNew,"col-lg-2 col-md-3 col-sm-3 hidden-xs",v.roomNumber,v.image, ndecodeURI(v.nickName),ndecodeURI(v.nickName),v.numbers);
                        }else if(k>12 && k <=18){
                            s_sml = Tools.stringFormat(compiliter.lithtmlNew,"col-lg-2 hidden-md hidden-sm hidden-xs",v.roomNumber,v.image, ndecodeURI(v.nickName),ndecodeURI(v.nickName),v.numbers);
                        }
                        srow.append(s_sml);
                    })
                    swrap.append(srow);
        ...
        compiliter.parseNewHotAnchors = function(url,type){
            Tools.getJson({
                ...
                if (hotData != null && hotData.length > 0) {
                    ...
                    if(window.navigator.userAgent.indexOf("MSIE")>=1){
                        bigData.roomNumber ="/guide.php?roomnumber=" + bigData.roomNumber;
                    }
