...
var showGift={
	show:function(gid,gnum,gwho){
		...
		var tmpgifturl="";
		if(cdn_domain==""){
			tmpgifturl="http://"+window.location.host+"/static_data/showGift/"+gid+".swf";
		}else{
			tmpgifturl=cdn_domain+"/static_data/showGift/"+gid+".swf";
		}
		swfobject.getObjectById('showGiftSwf').jsNewGift(tmpgifturl,gnum);
	},
	close:function(){
...
var Live= {
	...
	GiftSwitch:function(){
		if($("#giftSwitch").hasClass('giftanimation_on')){
			$("#giftSwitch").removeClass('giftanimation_on');
			$("#giftSwitch").addClass('giftanimation_off');
			...
			$("#giftSwitch").addClass('giftanimation_on');
			$("#giftSwitch").removeClass('giftanimation_off');
			Live.showFlashGift=true;
		}
	},
	flyMsgBoxSwitch:function(){
		if($("#flyMsgBoxSwitch").hasClass('giftanimation_on')){
			$("#flyMsgBoxSwitch").removeClass('giftanimation_on');
			$("#flyMsgBoxSwitch").addClass('giftanimation_off');
			$(".flyScreen").remove();
			Live.showflyMsgBoxSwitch=false;
		}else{
			$("#flyMsgBoxSwitch").addClass('giftanimation_on');
			$("#flyMsgBoxSwitch").removeClass('giftanimation_off');
	...
	setAdmin:function(){
		...
		swfobject.getObjectById("player").setadmin("setadmin",currentRoomNumber,currentOptUid);
	},
	blockIP:function(){
    ...
    appendPrivateChat:function(from,fromid,words,fontobj){
        ...
        $("#priChatList").append('<li style="'+getCssStyle(fontobj)+'">'+level1+' <a href="javascript:;" class="u" id="'+fromid+'">'+from+'</a>对你说：'+words+'</li>');
		var tmpli = $("#priChatList li").length;
		if(tmpli>50){
			$("#priChatList li:lt("+(tmpli-50)+")").remove();
		}
	...
	appendPrivateChatSelf:function(to,toid,words){
        ...
        words=replace_face(words);
		var tmp = $("#p"+toid);
		var level1=levelImg(toid,[toid,$.trim(tmp.find(".u a").html()),$.trim(tmp.find(".level").attr("class")).replace("level","").replace("lv",""),0]);
        if(level1==undefined)
        	level1="";
        $("#priChatList").append('<li style="'+getCssStyleMy()+'">你对'+level1+'<a href="javascript:;" class="u" id="'+toid+'">'+to+'</a>说：'+words+'</li>');
		var tmpli = $("#priChatList li").length;
		if(tmpli>50){
			$("#priChatList li:lt("+(tmpli-50)+")").remove();
		}
	...
	appendPublicChat:function(stamp,from,fromid,words,to,toid,fontobj){//from修改为了对像,to修改为了对像
        if(stamp==""){
            stamp=now();
        }
        else{
        ...
        if(to==undefined){
        	...
        	if(level1==undefined)
        		level1="";
            $("#pubChatList").append('<li style="'+getCssStyle(fontobj)+'">'+stamp+''+level1+'<a href="javascript:;" class="u" id="'+fromid+'">'+from+'</a>说:'+words+'</em></li>');
        }
        else{
			var toobj = to;
			to = toobj[1];
			var level1=levelImg(fromid,fromobj);
        	var level2=levelImg(toid,toobj);
        	if(level2==undefined)
        		level2="";
            $("#pubChatList").append('<li style="'+getCssStyle(fontobj)+'">'+stamp+''+level1+'<a href="javascript:;" class="u" id="'+fromid+'">'+from+'</a>对'+level2+'<a href="javascript:;" class="u" id="'+toid+'">'+to+'</a>说:'+words+'</em></li>');
        }
		var tmpli = $("#pubChatList li").length;
		if(tmpli>50){
			$("#pubChatList li:lt("+(tmpli-50)+")").remove();
		}
	...
	select_song:function(songid){
		...
		swfobject.getObjectById("player").sendGift(200,songid,"",currentShowerid,currentRoomNumber,currentShowid,escape(currentShower),currentUserID,escape(currentUserNickname),sofaid);
	},
    ...
    welcome:function(who,car){
		...
		if(who.substr(0,5)=="Guest"){
			if(guestshow=="1"){
				...
				who = "游客_"+who.substr(who.length-5,5);
			}
		}
		if(obj.userinfo.userid==currentUserID){//进入房间是自己，马上刷新列表
			UseList.p=1;
		...
		if(obj.car.state=="hide"){
			...
			if(obj.userinfo.userid!=currentUserID){
				//如果用户列表小于15个，或排序值大于第一页最后一个排序值，过5秒刷新列表
				if($("#userList").children().length<15 || parseInt(obj.userinfo.orderbyjs)>UseList.p1orderby){
					clearTimeout(UseList.welUploadList);
					UseList.welUploadList = setTimeout(function(){
						UseList.p=1;
						UseList.update_user_list();
					},5000);
				}
			}
			GuardInOut("in",obj.userinfo.userid);
		}
		var level1=levelImg(obj.userinfo.userid,[obj.userinfo.userid,who,obj.userinfo.richlevel,obj.userinfo.viplevel]);
		...
		if(obj.car.giftname!=""){//有座驾
			...
			if(carname!=""&&carimg!=""){
				...
				carMovie.show(obj.car.giftid+".swf",carname,who);
			}else{
	...
	bye:function(userid,usernumber){
		...
		GuardInOut("out",userid);
		$("#usernum").html(parseInt($("#usernum").html())-1);
		$("#u_manager #p"+userid+",#userList #p"+userid).remove();
    },
    ...
    giftAnnounce:function(from,fromid,to,toid,gid,giftnum,time,arr){
		...
		//if($("#gift"+gid).parent().hasClass('gln5') || $("#gift"+gid).parent().hasClass('gln6') || giftnum>50){
		if(parseInt(arr[12])>live_gift_announce_max){
			$("#movelist1").prepend('<li class="gali"><a target="_blank" href="/findroom-'+toid+'.html"><span class="rf_b">'+from+'</span> 给 <span class="rf_b"> '+to+'</span> 赠送了 '+giftnum+'个&nbsp;<img class="giftsmall gift_'+(gid)+'_x" src="/images/pixel.gif"></a></li>');
			$(".gali").last().remove();
			$("#giftShowPnl .atab .atab2").click();
		}
	},
    specialGiftAnnounce:function(from,fromid,to,toid,gid,giftname,giftnum,time){
		var giftname=$("#gift"+gid+" span").html();
	...
	sendGiftFail:function(r){
			...
			if(r==0){
				...
				Main.confirm('余额不足','您没有足够的'+money_name+'，请充值！','去充值',function(){
					window.open("/ucenter.php?ptype=pay");
				});
            }
			else if(r==2){
...
var carMovie={
	show:function(swf,carname,nickname){
		if(!Live.showFlashGift){return;}
		swfobject.embedSWF((cdn_domain+'/static_data/car/swf/'+swf),"flash-animate-panel","100%", "100%", "10.0", "", {},{wmode: "transparent",allowScriptAccess: "always"});
		$(".gift-flash-animate-area").append('<div class="gift-animate-tip"><span>隆重欢迎</span><span class="anim-color-yellow">'+nickname+'</span><span>开着</span><span class="anim-color-yellow">'+carname+'</span><span>进入主播间</span></div>');
		$(".gift-flash-animate-area").show();
		setTimeout(function(){$(".gift-animate-tip").css('bottom','60px')},1000);
	},
	stop:function(){
		$(".gift-flash-animate-area").html('<div id="flash-animate-panel"></div>');
		$(".gift-flash-animate-area").hide();
	}
}
var FilterStr = {
...
var Gift_show = {
    ...
    npos: function(k, d) {
        ...
        var X = $('#video').offset().left+50;
        var Y = $('#video').offset().top+5;
		//Y-=200;
        return {
            x: j+X,
            y: i+Y
...
var Chat={
	...
	fromServerGiftAnnounce:function(cmd){
        ...
        if(command=='SGG'){//送礼
			...
			//bf only
			//if(num>=999){
			//	PlySwf.push("恭喜 "+unescape(from)+" 送出 "+num+" 个 "+$("#gift"+arr[8]+" span").html()+" 为"+unescape(to)+"送上礼物跑道 让爱飞的更高");
			//}
			var award=arr[7].split('|');
			if(award[0]!=0){
				if(award[0]>=parseInt(_LUCK_MULT_ANN_)){
					//PlySwf.push("恭喜 "+unescape(from)+" 送出 "+num+" 个幸运 "+$("#gift"+arr[8]+" span").html()+" 共计中得 "+award[1]+" "+money_name);
					PlySwf.push("恭喜"+(from)+"在"+(to)+"的房间赠送幸运礼物,获得"+award[0]+"倍大奖,金额"+award[1]+money_name);
				}
				if(action_roomnumber==currentRoomNumber){
					$("#pubChatList").append('<li class="fontred">'+now()+"恭喜"+(from)+"在"+(to)+"的房间赠送幸运礼物,获得"+award[0]+"倍大奖,金额"+award[1]+money_name+"</li>");
				}
			}
			else{
				Live.giftAnnounce(unescape(from),fromid,unescape(to),toid,arr[8],num,now(),arr);
			}
		}
	},
    fromServer:function(cmd){
        ...
        if(command=='SFM'){//飞屏
			...
			if(unescape(arr[2])==currentUserNickname){
				updateBalance();
			}
        }
		else if(command=='SKK'){
		...
		/*else if(command=='BOT'){//踢出房间
			...
			arr[1]=arr[1].replace(/\\'/g,"'");
			$("#pubChatList").append('<li>'+now()+':系统公告: '+arr[1]+'</li>');
			if(arr[1].indexOf("成功开通守护")>=0){
				getGuardList();
			...
			if(fromid==currentUserID)
				Main.alert('点歌完成');
			Live.refreshSongRequest();
			$("#pubChatList").append('<li>'+now()+':<a href="javascript:;" class="u" id="'+fromid+'">'+unescape(from)+'</a> 点了一首：'+arr[7]+',送给主播</li>');
			PlySwf.push(unescape(from)+' 点了一首：'+arr[7]+',送给主播');
		}
			...
			var level1=levelImg(fromid,[fromid,from,fromobj[1],fromobj[2]]);
        	var level2=levelImg(toid,[toid,to,toobj[1],toobj[2]]);
			if(arr[9]=="tietiao"){
				var gname = $("#"+arr[8]+" a").html();
				$("#pubChatList").append('<li>'+now()+level1+':<a href="javascript:;" class="u" id="'+fromid+'">'+unescape(from)+'</a> 给 '+level2+'<a href="javascript:;" class="u" id="'+toid+'">'+unescape(to)+'</a> 贴上 '+gname+' 贴条</li>');
				...
				$("#pubChatList").append('<li>'+now()+level1+':<a href="javascript:;" class="u" id="'+fromid+'">'+unescape(from)+'</a>送 '+level2+'<a href="javascript:;" class="u" id="'+toid+'">'+unescape(to)+'</a>'+num+'个<img src="images/pixel.gif" class="giftsmall '+getFileNmae(gid)+'_x"></em></li>');
    ...
    getMsg:function(from,to,msg,fontobj){
        ...
        //var msg=obj[1];
        if(to=="ALL"){
            Live.appendPublicChat("",fromobj,fromobj[0],msg,undefined,undefined,fontobj);
        }
        else{
            var toobj=to.split('-',4);
            Live.appendPublicChat("",fromobj,fromobj[0],msg,toobj,toobj[0],fontobj);
        }
    },
    getPMsg:function(from,content,fontobj){
		...
		Live.appendPrivateChat(tmp,fromuid,content,fontobj);
    },
...
function showTaskWindow(c,obj,top){
	...
	if(obj!=""){//根据传入的对像对齐
		...
		if(IS_QQ=="1"){
			...
			var tmptop = (($(window).height())/2-(parseInt(285)/2));
		}
		$(".mis-overlay").css("left",((($(window).width())/2-(parseInt(500) /2))+parseInt(document.documentElement.scrollTop))+"px").css("top",tmptop+"px");
	}
}
//领取任务所得的钱
...
function levelImg(u,obj){//obj的值为，[19838] [88ddqq] [15] [0]  15级0vip
	if(isNaN(u))
		return "";
	var v = "";
	if(currentShowerid==u){
		//return '<img src="'+cdn_domain+'/apis/levelImg.php?u='+u+'&t=2" class=starTT>';
		v = '<em class="zlevel zlv'+currentShowstarlevel+'"></em>';
	}else{
		if(obj!=undefined){
			if(obj[2]==undefined){
				return '';
				...
				obj[2] = $.trim(obj[2]);
				v = '<em class="level lv'+obj[2]+'"></em>';
				obj[3] = $.trim(obj[3]);
				if(obj[3]>0){
					v+= '<img alt="VIP特权" src="/images/pixel.gif" class="vip'+obj[3]+'">';
				}
			}
		}else{
			v = '<img src="'+cdn_domain+'/apis/levelImg.php?u='+u+'&t=1" class=richTT>';
		}
	}
	return v;
}
...
$(function(){
	...
	$(".main_mic").click(function(e){//选择麦之成功之后广播一下
		$.post("roomactions.php?action=save_main_mic",{'main_mic':$(this).val()},function(r){
...
//发红包
function hongbao_fapage(){
	art.dialog.open('/ajax/hongbao_page.php?roomnumber='+currentRoomNumber, { 
		id:"hongbao_page",
		title:"发红包",
		lock:true, 
		width:400, 
...
//去发红包
function hongbao_fafsm(total,num){
	swfobject.getObjectById("player").hongbao("fa",currentRoomNumber,total,num);
}
//抢红包
function hongbao_qiang(sid){
	swfobject.getObjectById("player").hongbao("qiang",currentRoomNumber,sid,"");
}
//主播印象
function yinxiang_page(){
	art.dialog.open('/modules/yinxiang?roomnumber='+currentRoomNumber, { 
		id:"yinxiang_page",
		title:"主播印象",
		lock:true, 
		width:454, 
