...
switch($action){
	...
	break;
	case 'roundover':
		$db->Execute("insert into carrecord(dealerid,dealercj,result)values('$_GET[dealerid]','$_GET[dealercj]','$_GET[result]')");
		$numrow=$db->GetRow("select id,num from caraward order by id asc limit 1");
		if($numrow){
			echo (int)$numrow['num'];
			$db->Execute("delete from caraward where id='$numrow[id]'");
		}
		else{
			echo '0';
		}
